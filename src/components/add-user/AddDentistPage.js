...
export default function AddDentistPage() {
    ...
    const [formData, setFormData] = useState({
        ...
        birthdate: '', licenseNumber: '', specialization: '',
        email: '', phone: '',
    ...
    const getMaxDate = () => {
        const today = new Date();
        today.setFullYear(today.getFullYear() - 21); // Minimum age 21 for dentist
    ...
    const validateForm = () => {
        ...
        requiredFields.forEach(field => {
            if (!formData[field]) { newErrors[field] = "This field is required"; isValid = false; }
        });
        ...
        if (formData.birthdate && getAge(formData.birthdate) < 21) {
            newErrors.birthdate = "Dentist must be at least 21 years old"; isValid = false;
        }
            ...
            <div className={styles.formCard}>
                ...
                <form onSubmit={handleSubmit} noValidate>
                    ...
                    <div className={styles.row}>
                        ...
                        <div className={styles.formGroup}><label>LICENSE NO. <span style={{color: 'red'}}>*</span></label><input className={`${styles.inputField} ${errors.licenseNumber ? styles.errorBorder : ''}`} name="licenseNumber" value={formData.licenseNumber} onChange={(e) => { const val = e.target.value.replace(/[^0-9]/g, ''); setFormData({...formData, licenseNumber: val}); }} maxLength={7}/>{errors.licenseNumber && <span className={styles.errorText}>{errors.licenseNumber}</span>}</div>
